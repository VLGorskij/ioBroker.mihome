...
THSensor.prototype.getData = function (data, isHeartbeat) {
    let newData = false;
    let obj = {};
    const ts = Date.now();
    ...
    if (isHeartbeat) {
        const ts1 = Date.now();
        if (this.lastReply === undefined) {
            this.lastReply = ts1;
        }  
        const diff = ts1 - this.lastReply;
        this.hub.emit('warning', this.type + ' ; ' + this.sid + ' ; ' + 'diff = ' + diff );
        obj.heartbeat = diff;
        this.lastReply = ts1;
        newData = true;
    }
